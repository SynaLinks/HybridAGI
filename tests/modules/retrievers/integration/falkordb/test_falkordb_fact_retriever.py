...
def test_falkordb_fact_retriever():
    input_data = \
    [
        {
    ...
    fact_memory = FalkorDBFactMemory(index_name="test_fact_retriever", wipe_on_start=True)
    
    fact_memory.update(output_facts)
    
    fact_retriever = FalkorDBFactRetriever(
    ...
    result = fact_retriever(Query(query="godfather produced in"))
