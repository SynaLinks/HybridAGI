...
"""
    ...
    parser.parse(filename="SynaLinks.txt", content=text)
    assert len(memory.get_triplets("SynaLinks")) == 1

    parser.parse(filename="SynaLinks.txt", content=text, mode="deep")
    assert len(memory.get_triplets("SynaLinks")) == 4
