...
def test_exists(memory):
    memory.hybridstore.select_graph.return_value.query.return_value.result_set = [["result"]]
    assert memory.exists("test_index")

def test_exists_not_found(memory):
    memory.hybridstore.select_graph.return_value.query.return_value.result_set = []
    assert not memory.exists("test_index")

def test_clear(memory):
    memory.clear()
    memory.hybridstore.select_graph.return_value.delete.assert_called_once()
